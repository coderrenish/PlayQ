...
export const uportal_gb = {

  membershipBenefitsPage: {
    btn_start_application: () => page().getByRole('button', { name: 'Start Application' }),
    btn_retrive_draft: () => page().getByRole('button', { name: 'Retrieve Draft' }),
  },
  membershipPage: {
    btn_apply_manually: () => page().locator("//button[normalize-space(text())='Apply manually']"),
    btn_apply_with_myinfo: () => page().locator("button#mui-4"),
  }
};
